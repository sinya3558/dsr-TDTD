...
class RobotController(Node):
    def __init__(self):
        ...
        # 로봇 초기 위치 이동 (이 부분도 상황에 따라 process에서 호출하는게 안전합니다)
        self.init_robot()
        self.get_logger().info("RobotController 객체 생성 완료 (대기 중...)")
        # self.init_robot()
    ...
    def robot_control(self):
        if not self.get_keyword_client.wait_for_service(timeout_sec=1.0):
            self.get_logger().warn("Keyword 서비스가 아직 준비되지 않았습니다.")
            return
